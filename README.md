...
Market Agent is a private, read-only investment research dashboard that runs on Home Assistant Green. It checks daily market data, optionally researches current news with OpenAI, reads a Trading 212 portfolio with read-only credentials, creates an 80/15/5 risk-aware deployment plan, and sends a Home Assistant notification. It never places trades.
...
4. Enter your own OpenAI and Trading 212 credentials. Use a Trading 212 key with portfolio/account read access only—never order permissions.
